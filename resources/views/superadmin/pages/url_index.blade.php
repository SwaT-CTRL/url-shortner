...
@section('page-title', 'Global Shortened URLs')

@section('content')
<div style="margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 1rem;">
    <div>
        <h1 style="font-size: 1.25rem; font-weight: 700; margin-bottom: 0.5rem;">System-wide URLs</h1>
        <form action="{{ route('superadmin.urls.index') }}" method="GET" id="filterForm" style="display: flex; align-items: center; gap: 0.5rem;">
            ...
            <a href="{{ route('superadmin.urls.pdf', ['filter' => request('filter')]) }}" style="padding: 0.45rem 1rem; background: var(--primary); color: #fff; border-radius: 8px; font-size: 0.85rem; font-weight: 600; text-decoration: none; display: flex; align-items: center; gap: 0.5rem; transition: background 0.2s;">
            ...
            <tbody>
                @forelse($urls as $url)
                    <tr style="border-bottom: 1px solid var(--card-border);">
                        ...
                        <td style="padding: 1rem;">
                            <span style="color: var(--text);">{{ $url->generator->name ?? 'System' }}</span>
                            <br>
                            <small style="color: var(--text-muted); font-size: 0.7rem;">{{ class_basename($url->generator_type) }}</small>
                        </td>
